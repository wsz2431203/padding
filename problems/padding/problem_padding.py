...
class PADDINGDataset(Dataset):
    ...
    def __init__(self, filename=None, num_events=1000, num_samples=100000, num_category=3, time_window_length=100,
        ...
        if filename is not None:
            ...
            one_hot_cat = torch.zeros(num_samples, num_events,
                                      num_category).scatter_(2, cat_label, 1)
            event_type = torch.randint(2, (num_samples, num_events, 1))
            one_hot_event_type = torch.zeros(num_samples, num_events,
            ...
            data = torch.cat([t, one_hot_event_type, one_hot_cat, turn_time], dim=2)
