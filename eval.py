    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar):
        ...
        with torch.no_grad():
            if opts.decode_strategy in ('sample', 'greedy'):
                ...
                # Handle padding problem
                traced_init_embed = torch.jit.trace(model.init_embed, batch)
                traced_embedder = torch.jit.trace(model.embedder, model.init_embed(batch))
                from problems import PADDING
                i = 0
                graph_size = batch.size(-2)
                step_size = 50
                sequences = []
                while i < graph_size:
                    e = min(i + step_size, graph_size)
                    sub_batch = batch[:, i:e, :]
                    seq, costs = model.sample_many(sub_batch, batch_rep=batch_rep, iter_rep=iter_rep)
                    sequences.append(seq + i)
                    i += step_size
                sequences = torch.cat(sequences, dim=1)

                batch_size, graph_size, _ = batch.size()
                sorted_data = batch.gather(1, sequences.unsqueeze(-1).expand_as(batch))
                t = sorted_data[:, :, [0]]
                ...
                total_dist, _ = PADDING.get_costs(batch, sequences.gather(1, pi))
                
                costs = total_dist
                sequences = sequences.gather(1, pi)
