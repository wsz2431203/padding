...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is None:
            if self.problem.NAME == 'padding':
                self.dataset = self.problem.make_dataset(
                    num_events=self.opts.graph_size, num_samples=self.opts.val_size)
            else:
                self.dataset = self.problem.make_dataset(
    ...
    def epoch_callback(self, model, epoch):
        ...
        self.mean = rollout(self.model, self.dataset, self.opts).cpu().numpy().mean()
